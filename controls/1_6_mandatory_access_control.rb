...
control 'cis-dil-benchmark-1.6.2.1' do
  ...
  only_if('CIS level 2 and SELinux installed') do
    cis_level == 2 && (
  ...
  describe.one do
    %w(/boot/grub/menu.lst /boot/grub2/grub.cfg).each do |f|
      describe file(f) do
...
control 'cis-dil-benchmark-1.6.2.4' do
  ...
  desc  "The SETroubleshoot service notifies desktop users of SELinux denials through a userfriendly interface. The service provides important information around configuration errors, unauthorized intrusions, and other potential errors.\n\nRationale: The SETroubleshoot service is an unnecessary daemon to have running on a server, especially if X Windows is disabled."
  impact 1.0
...
control 'cis-dil-benchmark-1.6.3.1' do
  ...
  only_if('CIS level 2 and AppArmor installed') do
    cis_level == 2 && (
