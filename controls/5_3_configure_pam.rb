    ...
    describe.one do
      %w(common-password system-auth).each do |f|
        describe file("/etc/pam.d/#{f}") do
          ...
          its(:content) { should match(/^password required pam_cracklib\.so (\S+\s+)*dcredit=-[1-9][0-9]$/) }
          its(:content) { should match(/^password required pam_cracklib\.so (\S+\s+)*lcredit=-[1-9][0-9]$/) }
          its(:content) { should match(/^password required pam_cracklib\.so (\S+\s+)*ucredit=-[1-9][0-9]$/) }
          its(:content) { should match(/^password required pam_cracklib\.so (\S+\s+)*ocredit=-[1-9][0-9]$/) }
        end
      end
    end
#  end
