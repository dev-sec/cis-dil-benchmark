...
control 'cis-dil-benchmark-4.1.4' do
  ...
  describe.one do
    %w(/boot/grub/grub.cfg /boot/grub2/grub.cfg).each do |f|
      describe command("grep \"^\\s*linux\" #{f} | grep -v audit=1") do
        its('stdout') { should eq '' }
      end
    end
  end

  describe command('grep "^\s*kernel" /boot/grub/menu.lst | grep -v audit=1') do
    its('stdout') { should eq '' }
  end
end
...
control 'cis-dil-benchmark-4.1.5' do
  ...
  describe auditd do
    its('lines') { should include %r{-a (always,exit|exit,always) -F arch=b32 -S adjtimex -S settimeofday -S stime -k time-change} }
    its('lines') { should include %r{-a (always,exit|exit,always) -F arch=b32 -S clock_settime -k time-change} }
    its('lines') { should include %r{-w /etc/localtime -p wa -k time-change} }
  end

  if command('uname -m').stdout.strip == 'x86_64'
    describe auditd do
      its('lines') { should include %r{-a (always,exit|exit,always) -F arch=b64 -S adjtimex -S settimeofday -k time-change} }
      its('lines') { should include %r{-a (always,exit|exit,always) -F arch=b64 -S clock_settime -k time-change} }
    end
  end
end
...
control 'cis-dil-benchmark-4.1.7' do
  ...
  describe auditd do
    its('lines') { should include %r{-a (always,exit|exit,always) -F arch=b32 -S sethostname -S setdomainname -k system-locale} }
  ...
  if command('uname -m').stdout.strip == 'x86_64'
    describe auditd do
      its('lines') { should include %r{-a (always,exit|exit,always) -F arch=b64 -S sethostname -S setdomainname -k system-locale} }
    end
  end
end
...
control 'cis-dil-benchmark-4.1.9' do
  ...
  describe auditd do
    ...
    its('lines') { should include %r{-w /var/log/tallylog -p wa -k logins} }
  end
end

control 'cis-dil-benchmark-4.1.10' do
  title 'Ensure session initiation information is collected'
  desc  "Monitor session initiation events. The parameters in this section track changes to the files associated with session events. The file /var/run/utmp tracks all currently logged in users. All audit records will be tagged with the identifier \"session.\" The /var/log/wtmp file tracks logins, logouts, shutdown, and reboot events. The file /var/log/btmp keeps track of failed login attempts and can be read by entering the command /usr/bin/last -f /var/log/btmp. All audit records will be tagged with the identifier \"logins.\"\n\nRationale: Monitoring these files for changes could alert a system administrator to logins occurring at unusual hours, which could indicate intruder activity (i.e. a user logging in at a time when they do not normally log in)."
  impact 1.0
...
control 'cis-dil-benchmark-4.1.11' do
  ...
  describe auditd do
    its('lines') { should include %r{-a (always,exit|exit,always) -F arch=b32 -S chmod -S fchmod -S fchmodat -F auid\>=#{uid_min} -F auid!=4294967295 -k perm_mod} }
    its('lines') { should include %r{-a (always,exit|exit,always) -F arch=b32 -S chown -S fchown -S fchownat -S lchown -F auid\>=#{uid_min} -F auid!=4294967295 -k perm_mod} }
    its('lines') { should include %r{-a (always,exit|exit,always) -F arch=b32 -S setxattr -S lsetxattr -S fsetxattr -S removexattr -S lremovexattr -S fremovexattr -F auid\>=#{uid_min} -F auid!=4294967295 -k perm_mod} }
  end

  if command('uname -m').stdout.strip == 'x86_64'
    describe auditd do
      its('lines') { should include %r{-a (always,exit|exit,always) -F arch=b64 -S chmod -S fchmod -S fchmodat -F auid\>=#{uid_min} -F auid!=4294967295 -k perm_mod} }
      its('lines') { should include %r{-a (always,exit|exit,always) -F arch=b64 -S chown -S fchown -S fchownat -S lchown -F auid\>=#{uid_min} -F auid!=4294967295 -k perm_mod} }
      its('lines') { should include %r{-a (always,exit|exit,always) -F arch=b64 -S setxattr -S lsetxattr -S fsetxattr -S removexattr -S lremovexattr -S fremovexattr -F auid\>=#{uid_min} -F auid!=4294967295 -k perm_mod} }
    end
  end
end
...
control 'cis-dil-benchmark-4.1.12' do
  ...
  describe auditd do
    its('lines') { should include %r{-a (always,exit|exit,always) -F arch=b32 -S creat -S open -S openat -S truncate -S ftruncate -F exit=-EACCES -F auid\>=#{uid_min} -F auid!=4294967295 -k access} }
    its('lines') { should include %r{-a (always,exit|exit,always) -F arch=b32 -S creat -S open -S openat -S truncate -S ftruncate -F exit=-EPERM -F auid\>=#{uid_min} -F auid!=4294967295 -k access} }
  end

  if command('uname -m').stdout.strip == 'x86_64'
    describe auditd do
      its('lines') { should include %r{-a (always,exit|exit,always) -F arch=b64 -S creat -S open -S openat -S truncate -S ftruncate -F exit=-EACCES -F auid\>=#{uid_min} -F auid!=4294967295 -k access} }
      its('lines') { should include %r{-a (always,exit|exit,always) -F arch=b64 -S creat -S open -S openat -S truncate -S ftruncate -F exit=-EPERM -F auid\>=#{uid_min} -F auid!=4294967295 -k access} }
    end
  end
end
...
control 'cis-dil-benchmark-4.1.13' do
  ...
  describe auditd do
    command('find / -xdev \( -perm -4000 -o -perm -2000 \) -type f').stdout.split.each do |fn|
      its('lines') { should include %r{-a (always,exit|exit,always) -F path=#{fn} -F perm=x -F auid\>=#{uid_min} -F auid!=4294967295 -k privileged} }
    end
  end
end
...
control 'cis-dil-benchmark-4.1.14' do
  ...
  describe auditd do
    its('lines') { should include %r{-a (always,exit|exit,always) -F arch=b32 -S mount -F auid\>=#{uid_min} -F auid!=4294967295 -k mounts} }
  end

  if command('uname -m').stdout.strip == 'x86_64'
    describe auditd do
      its('lines') { should include %r{-a (always,exit|exit,always) -F arch=b64 -S mount -F auid\>=#{uid_min} -F auid!=4294967295 -k mounts} }
    end
  end
end
...
control 'cis-dil-benchmark-4.1.15' do
  ...
  describe auditd do
    its('lines') { should include %r{-a (always,exit|exit,always) -F arch=b32 -S unlink -S unlinkat -S rename -S renameat -F auid\>=#{uid_min} -F auid!=4294967295 -k delete} }
  end

  if command('uname -m').stdout.strip == 'x86_64'
    describe auditd do
      its('lines') { should include %r{-a (always,exit|exit,always) -F arch=b64 -S unlink -S unlinkat -S rename -S renameat -F auid\>=#{uid_min} -F auid!=4294967295 -k delete} }
    end
  end
end
...
control 'cis-dil-benchmark-4.1.16' do
  ...
  desc  "Monitor scope changes for system administrations. If the system has been properly configured to force system administrators to log in as themselves first and then use the sudo command to execute privileged commands, it is possible to monitor changes in scope. The file /etc/sudoers will be written to when the file or its attributes have changed. The
...
control 'cis-dil-benchmark-4.1.18' do
  ...
  if command('uname -m').stdout.strip == 'x86_64'
    describe auditd do
      its('lines') { should include %r{-a (always,exit|exit,always) -F arch=b64 -S init_module -S delete_module -k modules} }
    end
  else
    describe auditd do
      its('lines') { should include %r{-a (always,exit|exit,always) -F arch=b32 -S init_module -S delete_module -k modules} }
    end
  end
end
...
control 'cis-dil-benchmark-4.1.19' do
  ...
  describe auditd do
    its('lines') { should include %r{-e 2} }
  end
end
